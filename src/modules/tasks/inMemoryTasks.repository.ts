...
export class InMemoryTasksRepository implements TasksRepository {
  ...
  async findMany(params: {
    ...
    if (params.done !== undefined) {
      filteredTasks = filteredTasks.filter((task) => task.done === params.done);
    }
    ...
    if (params.cursor) {
      ...
      if (Number.isInteger(decodedCursor) && decodedCursor >= 0) {
        startIndex = decodedCursor;
      }
    }

    const items = filteredTasks.slice(startIndex, startIndex + params.limit);
